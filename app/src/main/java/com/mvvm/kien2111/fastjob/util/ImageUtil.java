...
/**
 * Created by WhoAmI on 02/05/2018.
 */

public final class ImageUtil {
    private static final String APP_IMAGE_DIR = "/app_img";
    ...
    public static String saveBitmap(Bitmap image,String id){
    ...
    public static int getTintImageColor(Profile profile){
        switch (profile.getPriority()){
            case BASIC:
                return Color.parseColor(mColorMaterial[0]);
            case MEDIUM:
                return Color.parseColor(mColorMaterial[1]);
            case PREMIUM:
                return Color.parseColor(mColorMaterial[2]);
            default:
                return Color.parseColor("#ffff");
        }
    }

    public static BitmapDescriptor bitmapDescriptorFromVector(Context context, @DrawableRes int vectorDrawableResourceId) {
        Drawable background = ContextCompat.getDrawable(context, R.drawable.ic_flag_map_marker);
    ...
    public static String[] mColorMaterial = {
            "#1e88e5","#3f51b5","#FFEB3B"
    };

    public static int randomColorBackground (){
        return Color.parseColor ("#"+mColors[new Random().nextInt(234)]);
    }
